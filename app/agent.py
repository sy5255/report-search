...
RAG_SYNTHESIS_STYLE_RULES = (
    "모든 답변은 100% 한국어로, 자연스럽게 연결된 서술형 문단으로 작성하세요. "
    "문서 요약 시 마크다운 표(| 기호)는 사용하지 마세요. "
    "주제별로 '### 💡 [주제]' 소제목을 달고, 그 아래에는 근거 문서 내용을 바탕으로 원리·원인·함의를 "
    "매끄럽게 이어지는 서술형 문단으로 설명하세요. 한 문장씩 뚝뚝 끊어 나열하지 말고 문장을 자연스럽게 "
    "연결하세요. 핵심을 특별히 강조하고 싶을 때만 인용구(> )를 절제해서 사용하세요."
)
